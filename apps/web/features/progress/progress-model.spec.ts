...
describe('stub snapshot', () => {
  const stub = buildProgressStub(NOW);

  it('materializes example completions relative to Shanghai today', () => {
    expect(stub.today).toBe(TODAY);
    expect(stub.completions[0]?.title).toBe('The Hidden World of Oceans');
    expect(stub.completions).toHaveLength(8);
  });

  it('counts five completions in the default 30-day window', () => {
    const selected = windowForTab('30', stub.today);
    const rows = completionsInWindow(stub.completions, selected);
    expect(rows).toHaveLength(5);
    expect(rows[0]?.title).toBe('The Hidden World of Oceans');
    expect(progressHeadline(selected, stub.today, 5)).toBe('这 30 天里，你读完了 5 篇。');
  });
  ...
  it('counts a current run of learning days without calling it a streak to keep', () => {
    expect(consecutiveLearningDays(stub.today, stub.activity)).toBe(4);
    expect(stub.portrait.consecutiveDays).toBe(4);
    expect(stub.portrait.completedArticles).toBe(8);
    expect(stub.portrait.learningDays).toBeGreaterThan(80);
    expect(progressAdvice(stub.portrait)).toContain('断几天也没关系');
  });
});
...
describe('contribution heatmap', () => {
  it('builds 53 Sunday-start weeks and marks the selected date range', () => {
    ...
    const stub = buildProgressStub(NOW);
    const selected = windowForTab('30', stub.today);
    const weeks = buildHeatmapWeeks(stub.today, stub.activity, selected);
    ...
    expect(todayCell).toMatchObject({ date: TODAY, future: false, isInRange: true });
    ...
    expect(countLearningDaysInWindow(stub.activity, selected)).toBeGreaterThan(0);
  });
});
