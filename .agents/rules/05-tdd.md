...
- Never truncate, drop, or clear an entire database table in a test or test teardown
- Never delete rows without an exact scope key set created by the same test
- Never use broad filters as cleanup scope, including title, email prefix, date range, status, author, or "looks unique" predicates
...
- Allowed scope keys are primary keys, explicit foreign keys, or test-local prefixes that were created and recorded inside the same test body
- "Created and recorded" means the test captured the exact IDs, paths, or job IDs it wrote
- A cleanup condition based on human-readable text alone is not a valid scope boundary
- If the test cannot name the exact records it created, the test must not write persistent state

Forbidden cleanup patterns:

- `truncate table ...`
- `delete from ...` without an exact `where id in (...)` or equivalent recorded key set
- `query().delete()` on a table unless the query is constrained to the current test-owned IDs
- `where('title', 'like', ...)`, `where('email', 'like', ...)`, `where('createdAt', ...)`, or any other broad match cleanup
- `redis.flushall()`, `redis.flushdb()`, or any global cache reset
- Deleting files or queue jobs by broad prefix unless the prefix was generated by the same test and the test owns the namespace

Boundary rule:

- A test may leave behind no state outside its own namespace when it exits, even if it fails halfway through
- If a test would require cleanup outside its own namespace, the test design is invalid and must be rewritten
