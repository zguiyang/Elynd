...
import { chapterStatusLabel } from '@/features/book-detail/book-detail-model';
import { cn } from '@/lib/utils';

/** Desktop prototype shows ~5 rows then “展开全部章节”. */
      ...
      {label ? (
        <span
          className={cn(
            ...
            isCurrent ? 'font-semibold text-primary' : isRead ? 'text-muted-foreground' : 'text-muted-foreground',
          )}
        >
          {label}
        </span>
...
function ChapterIcon({ status }: { status: BookChapter['status'] }) {
  ...
  if (status === 'current') {
    // Lucide PlayCircle + fill paints triangle the same color → looks like a solid disc.
    // Compose filled disc + contrasting play mark (Material play_circle FILL).
    return (
