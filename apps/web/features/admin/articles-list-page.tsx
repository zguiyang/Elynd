...
import { type ArticleStatus, MOCK_ARTICLES } from '@/features/admin/articles-mock-data';
...
export function ArticlesListPage() {
  ...
  const filtered = useMemo(() => {
    ...
    return MOCK_ARTICLES.filter((article) => article.status === status);
  }, [status]);

  const totalPages = Math.max(1, Math.ceil(filtered.length / ADMIN_ARTICLES_PAGE_SIZE));
  const safePage = Math.min(page, totalPages);
  const pageItems = filtered.slice((safePage - 1) * ADMIN_ARTICLES_PAGE_SIZE, safePage * ADMIN_ARTICLES_PAGE_SIZE);
  ...
  const emptyTitle = filtered.length === 0 && MOCK_ARTICLES.length > 0 ? '当前筛选下没有文章' : '还没有文章';
  const emptyDescription =
    filtered.length === 0 && MOCK_ARTICLES.length > 0
      ? '试试切换状态筛选，或新建一篇短文。'
      : '点「新建文章」开始粘贴内容。';
    ...
    <div className="motion-safe:animate-in motion-safe:fade-in motion-safe:slide-in-from-bottom-3 motion-safe:duration-700 mx-auto max-w-6xl">
      ...
      <div className="mt-8 flex flex-col items-center justify-between gap-4 sm:flex-row">
        <p className="text-sm text-muted-foreground">
          共 {filtered.length} 篇 · 第 {safePage} / {totalPages} 页 · 每页 {ADMIN_ARTICLES_PAGE_SIZE}
        </p>
